...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/size-~100KB-lightgrey" alt="Size">
...
**SingPilot** 走了另一条路：**100 KB 的纯 PowerShell 脚本，给 sing-box 配上终端菜单、自动健康守护、零依赖**。
...
| 偏好地区 | `[12]` 按 `p` 钉住某地区（如 SG）；只要不挂就一直用它，挂了自动退、恢复自动回 |
...
Invoke-RestMethod "$api/proxies"                    # 所有节点和分组

# 看当前选中的节点（分组名含中文/emoji，URL 要转义）
$g = [uri]::EscapeDataString("🌏️主代理")
(Invoke-RestMethod "$api/proxies/$g").now

# 测某个节点延迟
$n = [uri]::EscapeDataString("SG-2")
(Invoke-RestMethod "$api/proxies/$n/delay?timeout=5000&url=https%3A%2F%2Fwww.gstatic.com%2Fgenerate_204").delay

# 切换节点 —— body 必须显式转 UTF-8 字节！
# PowerShell 5.1 的 Invoke-RestMethod 不会按 UTF-8 编码字符串 body，
# 分组名含中文/emoji 时会传成乱码，sing-box 报 "Selector update error: not found"。
$body = [Text.Encoding]::UTF8.GetBytes('{"name":"SG-2"}')
Invoke-RestMethod "$api/proxies/$g" -Method PUT -Body $body -ContentType "application/json"

# 按偏好地区自动选节点（非交互）
powershell -File scripts\speedtest.ps1 -Enforce
```
...
├── diagnose.bat            ← 一键网络诊断
├── sing-box.exe            ← 你放进来
├── config.json             ← 你放进来（每次更新订阅会被覆盖）
├── config.local.json       ← 你的本地定制，更新后自动合并回 config.json
...
│   ├── start.ps1 / stop.ps1
│   ├── status.ps1           ← 诊断面板
│   ├── update.ps1           ← 订阅更新（含覆盖层合并 + 回滚）
│   ├── watchdog.ps1         ← 健康守护 + 偏好地区执行
│   ├── autostart.ps1        ← 开机自启
│   ├── sysproxy.ps1         ← 系统代理开关
│   ├── dnstool.ps1          ← DNS 工具
│   ├── logview.ps1          ← 日志查看
...
- **config.json** (从服务商获取)
- **管理员权限** —— TUN 模式、`[6]` 看门狗、`[7]` 开机自启都需要（后两者会注册 SYSTEM 计划任务）。右键 `manage.bat` → 以管理员身份运行
...
**Q: 更新订阅会不会把我的配置改动冲掉？**
A: 会——订阅是服务商原样生成的。所以定制要写进 `config.local.json`，每次更新后会自动合并回去。直接改 `config.json` 撑不过下一次更新。

**Q: 升级内核（`[14]`）会不会把代理搞挂？**
A: 不会。它在替换前先用**新内核**校验你现有的配置，过不了就不装（内核原封不动）；装完启动失败会自动回滚旧内核。旧版本备份在 `backup/sing-box-<版本>.exe`。

**Q: `[6]` / `[7]` 装完提示成功，但菜单还显示没装？**
A: 检查一下是不是没用管理员身份。这两个要注册 SYSTEM 计划任务，普通权限会被拒。另外普通权限窗口**查询**这些任务同样会被拒（`schtasks` 返回 `Access is denied`），工具箱能区分"被拒"和"不存在"，所以状态显示是准的。
...
**SingPilot** is an AI-native control plane for [sing-box](https://github.com/Sagernet/sing-box) — a lightweight launcher and health guardian in ~100 KB of pure PowerShell.
...
- Administrator — needed for TUN mode and for `[6]`/`[7]`, which register SYSTEM scheduled tasks

No other dependencies.

### Notes for AI agents

- The Clash API lives at `http://127.0.0.1:9090` (port is read from `config.json`, not hardcoded).
- Group names often contain CJK/emoji. URL-escape them, **and send PUT/PATCH bodies as UTF-8 bytes** — PowerShell 5.1's `Invoke-RestMethod` does not encode string bodies as UTF-8, and sing-box will answer `Selector update error: not found`.
- `scripts/speedtest.ps1 -Enforce` selects a node by preferred region non-interactively; every other script is interactive.
- `sing-box check` passing does **not** mean the service will start — some errors only surface at startup. Always verify the process is alive after a config change.
